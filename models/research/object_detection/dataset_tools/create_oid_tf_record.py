...
tf.flags.DEFINE_integer('each_record_size', 100, 'Size of each record file. (Mb)')

FLAGS = tf.flags.FLAGS


def open_output_tfrecord(exit_stack, base_path, idx):
  ...
  Args:
    ...
    idx: The number of idx

  Returns:
    The list of opened TFRecords. Position k in the list corresponds to shard k.
  """
  tf_record_output_filename = '{}-{:05d}'.format(base_path, idx)
  tfrecord = \
    exit_stack.enter_context(
      tf.python_io.TFRecordWriter(tf_record_output_filename))
  return tfrecord
...
def main(_):
  ...
  with contextlib2.ExitStack() as tf_record_close_stack:
    file_idx = 0
    output_tfrecord = \
      open_output_tfrecord(
        tf_record_close_stack, FLAGS.output_tf_record_path_prefix, file_idx)
    total_size = 0
    total_file_cnt = 0
    for counter, image_data in enumerate(all_annotations.groupby('ImageID')):
      tf.logging.log_every_n(tf.logging.INFO, 'Processed total:%d images...', 1000,
                             counter)
      ...
      if tf_example:
        ...
        total_size += (len(tf_example.SerializeToString())+16)
      
      if total_size > FLAGS.each_record_size*1024*1024: 
        file_idx += 1
        total_size = 0
        output_tfrecord = \
          open_output_tfrecord(
    ...
    print("total_file_cnt: %d" % total_file_cnt)
